  ...
  const handleSelect = (comboId, subTypeName, groupLabel, subTypeId) => {
    ...
    if (comboId === COMBINATIONS.SELECT_MINOR) {
      ...
      const autoSelectedIds = CS_OPTION_IDS.filter(
        (id) => id !== manuallyChosen && !EXCLUDED_SUB_TYPES.includes(id),
      );
    ...
    if (
      Object.keys(newComboSelections).length === getRequiredSelections(comboId)
    ) {
      setOpenCombo(null);
      setOpenSub(null);
    }
          ...
          {Object.keys(selected).length > 0 ? (
            <div className="selected-combination-display">
              <span>
                ...
                {
                  combinations.find(
                    (c) => c.id.toString() === Object.keys(selected)[0],
                  )?.name
                }
              </span>
                      ...
                      .filter(
                        (group) =>
                          ...
                          group.options.some((opt) =>
                            CS_MINOR_IDS.includes(opt.sub_type_id),
                          ),
                      )
                      .map((group, i) => (
                        <div
