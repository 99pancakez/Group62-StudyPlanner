...
const CreateCourseModal = ({
  ...
  const groupedSubTypeOptions =
    selectedTypes.length > 0
      ? selectedTypes.map((type) => {
          const seen = new Set();
          const filtered = safeSubTypes
            .filter((st) => {
            ...
            <div className="form-grid">
                ...
                <input
                  ...
                  onBlur={() => {
                    ...
                    if (!/^\d{6}$/.test(courseId))
                      err.courseId =
                        "Course ID must be 6 digits (e.g., 012345)";
                    else if (
                      allCourseCodes.some((c) => c.course_id === courseId)
                    )
                      err.courseId = "Course ID already exists";
              ...
              <label>
                ...
                <input
                  ...
                  onBlur={() => {
                    const err = { ...validationErrors };
                    if (!/^[A-Z]{4}\d{4}$/i.test(courseCode))
                      err.courseCode =
                        "Course Code must be in the format ABCD1234";
                    else if (
                      allCourseCodes.some((c) => c.course_code === courseCode)
                    )
                      err.courseCode = "Course Code already exists";
              ...
              <label>
                ...
                <input
                  ...
                  onBlur={() => {
                    const err = { ...validationErrors };
                    if (!courseTitle.trim())
                      err.courseTitle = "Course Title is required";
                    else delete err.courseTitle;
                    setValidationErrors(err);
                  }}
                />
                {validationErrors.courseTitle && (
                  <div className="error">{validationErrors.courseTitle}</div>
                )}
              </label>
              <label>
                Web URL
            ...
            <div className="availability-section">
              <label>
                <input
                  type="checkbox"
                  checked={semester1}
                  onChange={() => setSemester1(!semester1)}
                />
                Semester 1
              </label>
              <label>
                <input
                  type="checkbox"
                  checked={semester2}
                  onChange={() => setSemester2(!semester2)}
                />
                Semester 2
              </label>
              <label>
          ...
          {tab === 3 && (
            <div className="review-summary">
              ...
              <ul>
                ...
                <li>
                  <strong>Web URL:</strong> {webUrl || "N/A"}
                </li>
                <li>
            ...
            {tab === 3 && (
              <button
                onClick={async () => {
                  const finalErrors = {};
                  if (!/^\d{6}$/.test(courseId))
                    finalErrors.courseId =
                      "Course ID must be 6 digits (e.g., 012345)";
                  else if (allCourseCodes.some((c) => c.course_id === courseId))
                    finalErrors.courseId = "Course ID already exists";
                  if (!/^[A-Z]{4}\d{4}$/i.test(courseCode))
                    finalErrors.courseCode =
                      "Course Code must be in the format ABCD1234";
                  else if (
                    allCourseCodes.some((c) => c.course_code === courseCode)
                  )
                    finalErrors.courseCode = "Course Code already exists";
                  if (!courseTitle.trim())
                    finalErrors.courseTitle = "Course Title is required";
                  ...
                  try {
                    ...
                    if (!response.ok) {
                      ...
                      if (message.includes("Course ID already exists")) {
                        setServerErrors({
                          courseId: "Course ID already exists",
                        });
                      } else if (
                        message.includes("Course Code already exists")
                      ) {
                        setServerErrors({
