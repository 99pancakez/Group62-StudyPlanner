...
export default function HistoryPage() {
  ...
  const totalPages =
    rowsPerPage === "All" ? 1 : Math.ceil(allGroups.length / rowsPerPage);
        ...
        <table className="history-table">
          ...
          <tbody>
            {paginatedGroups.map((group, index) => {
              ...
              const isCreateGroup =
                courseFields.every((field) => fieldNames.includes(field)) &&
                group.every(
                  (log) => log.old_value === "" || log.old_value === null,
                );

              const isDeleteGroup =
                courseFields.every((field) => fieldNames.includes(field)) &&
                group.every(
                  (log) => log.new_value === "" || log.new_value === null,
                );
                ...
                return (
                  <React.Fragment key={groupKey}>
                    ...
                    >
                      ...
                      >
                        {expandedGroups[groupKey] ? "▼" : "▶"} Course{" "}
                        {isCreateGroup ? "Created" : "Deleted"}:{" "}
                        {first.new_value || first.old_value || "Unknown"} —{" "}
                        {new Date(first.time_stamp).toLocaleString()}
                      </td>
                    </tr>
