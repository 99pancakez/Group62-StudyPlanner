...
db.CombinationGroup = require("./models/combination_group")(
  db.sequelize,
  DataTypes,
);
db.ComboGroupTypeMapping = require("./models/combo_group_type_mapping")(
  db.sequelize,
  DataTypes,
);
db.CombinationGroupMapping = require("./models/combination_group_mapping")(
  db.sequelize,
  DataTypes,
);
db.RequisiteRule = require("./models/requisite_rule")(db.sequelize, DataTypes);
db.RequisiteGroup = require("./models/requisite_group")(
  db.sequelize,
  DataTypes,
);
db.Requisite = require("./models/requisite")(db.sequelize, DataTypes);

// Define associations

// admin to history (one-to-many)
db.Admin.hasMany(db.History, {
  foreignKey: { name: "admin_id", allowNull: false },
});
db.History.belongsTo(db.Admin, {
  foreignKey: { name: "admin_id", allowNull: false },
});

// admin to program_plan (one-to-many)
db.Admin.hasMany(db.ProgramPlan, {
  foreignKey: { name: "admin_id", allowNull: false },
});
db.ProgramPlan.belongsTo(db.Admin, {
  foreignKey: { name: "admin_id", allowNull: false },
});

// type to sub_type (one-to-many)
db.Type.hasMany(db.SubType, {
  foreignKey: { name: "course_type_id", allowNull: false },
});
...
db.SubType.hasMany(db.CourseType, {
  foreignKey: { name: "sub_type_id", allowNull: false },
});
db.CourseType.belongsTo(db.SubType, {
  foreignKey: { name: "sub_type_id", allowNull: false },
});

// course to course_type (one-to-many)
db.Course.hasMany(db.CourseType, {
  foreignKey: { name: "course_id", allowNull: false },
});
db.CourseType.belongsTo(db.Course, {
  foreignKey: { name: "course_id", allowNull: false },
});
...
db.CourseAvailability.belongsTo(db.Availability, {
  foreignKey: { name: "semester_id", allowNull: false },
  as: "availability",
});

// course to course_availability (one-to-many)
db.Course.hasMany(db.CourseAvailability, {
  foreignKey: { name: "course_id", allowNull: false },
  as: "courseAvailabilities",
});
db.CourseAvailability.belongsTo(db.Course, {
  foreignKey: { name: "course_id", allowNull: false },
  as: "course",
});

// program_plan to program_course (one-to-many)
db.ProgramPlan.hasMany(db.ProgramCourse, {
  foreignKey: { name: "program_code", allowNull: false },
});
db.ProgramCourse.belongsTo(db.ProgramPlan, {
  foreignKey: { name: "program_code", allowNull: false },
});

// course to program_course (one-to-many)
db.Course.hasMany(db.ProgramCourse, {
  foreignKey: { name: "course_id", allowNull: false },
});
db.ProgramCourse.belongsTo(db.Course, {
  foreignKey: { name: "course_id", allowNull: false },
});

// program_plan to history (one-to-many)
db.ProgramPlan.hasMany(db.History, {
  foreignKey: { name: "program_code", allowNull: true },
});
db.History.belongsTo(db.ProgramPlan, {
  foreignKey: { name: "program_code", allowNull: true },
});

// course to history (one-to-many)
db.Course.hasMany(db.History, {
...
db.ProgramPlan.hasMany(db.Combination, {
  foreignKey: { name: "program_code", allowNull: false },
});
db.Combination.belongsTo(db.ProgramPlan, {
  foreignKey: { name: "program_code", allowNull: false },
});

// Combination → CombinationGroupMapping (1:M)
db.Combination.hasMany(db.CombinationGroupMapping, {
  foreignKey: { name: "combination_id", allowNull: false },
});
db.CombinationGroupMapping.belongsTo(db.Combination, {
  foreignKey: { name: "combination_id", allowNull: false },
});

// CombinationGroup → CombinationGroupMapping (1:M)
db.CombinationGroup.hasMany(db.CombinationGroupMapping, {
  foreignKey: { name: "combo_group_id", allowNull: false },
});
db.CombinationGroupMapping.belongsTo(db.CombinationGroup, {
  foreignKey: { name: "combo_group_id", allowNull: false },
});

// CombinationGroup → ComboGroupTypeMapping (1:M)
db.CombinationGroup.hasMany(db.ComboGroupTypeMapping, {
  foreignKey: { name: "combo_group_id", allowNull: false },
});
db.ComboGroupTypeMapping.belongsTo(db.CombinationGroup, {
  foreignKey: { name: "combo_group_id", allowNull: false },
});

// Type → ComboGroupTypeMapping (1:M)
db.Type.hasMany(db.ComboGroupTypeMapping, {
  foreignKey: { name: "course_type_id", allowNull: false },
});
db.ComboGroupTypeMapping.belongsTo(db.Type, {
  foreignKey: { name: "course_type_id", allowNull: false },
});

// course (as target) to requisite_rule (one-to-many)
db.Course.hasMany(db.RequisiteRule, {
  foreignKey: { name: "target_course_id", allowNull: false },
  as: "requisite_rules",
});
db.RequisiteRule.belongsTo(db.Course, {
  foreignKey: { name: "target_course_id", allowNull: false },
  as: "target_course",
});

// requisite_rule to requisite_group (one-to-many)
db.RequisiteRule.hasMany(db.RequisiteGroup, {
  foreignKey: { name: "rule_id", allowNull: false },
  as: "groups",
});
db.RequisiteGroup.belongsTo(db.RequisiteRule, {
  foreignKey: { name: "rule_id", allowNull: false },
});
