...
exports.getCoursesByProgram = async (req, res) => {
  try {
    ...
    if (!program) {
      ...
      return res
        .status(404)
        .json({ success: false, message: `Program ${programCode} not found` });
    }
    ...
    const formattedCourses = await Promise.all(
      programCourses.map(async (programCourse) => {
        ...
        const semesters =
          course.courseAvailabilities?.map(
            (ca) => ca.availability?.semester_name,
          ) || [];
  ...
  try {
    ...
    if (course_code !== undefined && course.course_code !== course_code) {
      await createHistory({
        adminId,
        courseId,
        fieldName: readable("course_code"),
        oldValue: course.course_code,
        newValue: course_code,
      });
    ...
    if (course_title !== undefined && course.course_title !== course_title) {
      await createHistory({
        adminId,
        courseId,
        fieldName: readable("course_title"),
        oldValue: course.course_title,
        newValue: course_title,
      });
    ...
    if (web_url !== undefined && course.web_url !== web_url) {
      await createHistory({
        adminId,
        courseId,
        fieldName: readable("web_url"),
        oldValue: course.web_url,
        newValue: web_url,
      });
      ...
      for (const existing of existingCourseTypes) {
        if (!newSubTypeIds.includes(existing.sub_type_id)) {
          ...
          await CourseType.destroy({
            where: { course_id: courseId, sub_type_id: existing.sub_type_id },
          });
    ...
    if (Array.isArray(course_type)) {
      ...
      const toAddTypes = dedupedTypes.filter(
        (t) => !existingTypeNames.includes(t),
      );
      const toRemoveTypes = existingTypeNames.filter(
        (t) => !dedupedTypes.includes(t),
      );
...
exports.getCourseTypes = async (req, res) => {
  try {
    ...
    if (!types || types.length === 0) {
      ...
      return res
        .status(404)
        .json({ success: false, message: "No course types found" });
    }
...
exports.deleteCourse = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Server error during course deletion" });
  }
};
  ...
  try {
    ...
    if (!sub_type_name || !course_type) {
      return res.status(400).json({
        success: false,
        message: "Missing sub_type_name or course_type",
      });
    }
    ...
    if (existing) {
      return res
        .status(400)
        .json({ success: false, message: "Sub Type already exists" });
    }
...
exports.getAllCourseCodes = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Failed to fetch course codes" });
  }
};
...
exports.createCourse = async (req, res) => {
  try {
    ...
    if (!Array.isArray(sub_type) || sub_type.length === 0) {
      return res
        .status(400)
        .json({ success: false, message: "At least one Sub Type is required" });
    }
    ...
    const subTypeRecords = allSubTypes.filter(
      (st) =>
    ...
    if (subTypeRecords.length === 0) {
      return res.status(400).json({
        success: false,
        message: "No matching Sub Types found for selected Course Types",
      });
    }
    ...
    await createHistory({
      ...
      oldValue: null,
      newValue: newCourse.prerequisite ? "true" : "false",
    });
