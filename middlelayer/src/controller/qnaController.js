      ...
      const plainCourses = courses.map((course) => {
        ...
        course_types.forEach((ct) => {
          ...
          combo_group_type_mappings.forEach((mapping) => {
            ...
            const existingGroup = subTypeMap[subTypeId].type.combo_groups.find(
              (g) => g.combo_group_id === group.combo_group_id,
            );
            if (!existingGroup) {
            ...
            const comboGroup = subTypeMap[subTypeId].type.combo_groups.find(
              (g) => g.combo_group_id === group.combo_group_id,
            );
            group.combination_group_mappings?.forEach((mapping) => {
              ...
              if (
                combination &&
                !comboGroup.combinations.some(
                  (c) => c.combination_id === combination.combination_id,
                )
              ) {
